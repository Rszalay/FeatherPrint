...
---
*Rick Szalay, 10 June 26 — revised 01 July 26*
...
![Flare — Profile](images/Flare-Profile.PNG)

The Flare Rim profile is defined in the Anchor Point UCS, symmetric about the y-axis, with the anchor at x = 0 on the OML (y = 0) — not on the Flange's current innermost-Wall face. The canonical shape is a flat-bottomed channel: the profile runs flush along the innermost-Wall line until it nears the anchor, then breaks away at two departure points and drops through a filleted corner on each side to a flat channel bottom, where the colliding feature's crossover sits.

The channel's governing dimensions are parametrized rather than fixed, because both the required width and the required depth depend on context:

- **Width (W)** is the flat span between the two departure points: W = 2w when the Flare resolves a single Stringer anchor, or W = 3w when it resolves a Lacing anchor (matching the Lacing's own total lateral span of 3.0w).
- **Depth (D)** and **corner radius (R)** are both derived per Layer from the Flange's current Wall-stack thickness T at that Layer (T follows the build-up rule in the Flange section: 1.5w at the first ramp Layer, +0.5w per Layer thereafter). D and R are chosen so the crossover always lands at a fixed 2.5w below the OML — the same depth as an unmodified Stringer Trace or Lacing Trace — regardless of how much of that depth the Flange's own Walls already occupy:

  > D = 2.5w − T
  >
  > R = 0.75w × (D / 2.5w)

  R scales with D (rather than staying fixed) so the channel's proportions hold as it shallows; this keeps the fillets from overtaking an already-shallow channel at Layers where T approaches 2.5w. The canonical drawing shows the unscaled reference case — T = 0, giving D = 2.5w and R = 0.75w — the geometry is dimensioned in this reference case and then scaled per-Layer by the formulas above.

The channel is formed by two quarter-circle fillets bridging the flat innermost-Wall line to a flat bottom, with no intervening geometry:

| Segment | Centre | Radius | Direction | Start | End |
| --- | --- | --- | --- | --- | --- |
| Left fillet | (−(W/2 − R), −T − R) | R | CW | 90° | 180° |
| Bottom | — | — | — | (−(W/2 − R), −T − D) | (+(W/2 − R), −T − D) |
| Right fillet | (+(W/2 − R), −T − R) | R | CW | 0° | 90° |

The left departure point — where the Flare breaks from the flat innermost-Wall line — is (−W/2, −T); the right departure point is (+W/2, −T). Each fillet is tangent to the flat innermost-Wall line above and to the flat channel bottom below, so the transition from ordinary Flange Wall to Flare Rim, and back again, is smooth at both departure points.

This arc construction (tangent quarter-circles bridging two parallel flats at width W and depth D) is the simplest shape consistent with the dimensions in the reference drawing, and should be checked against the source CAD before implementation — the drawing does not by itself confirm fillet centre placement or tangency to sub-w precision.
...
The Developed view shows a Stringer tube approaching a Flange zone. Below the zone, the Stringer's ordinary Traces run against a flat, undisturbed OML/innermost-Wall pair. Entering the zone, successive Flare Rims shallow in step with the Flange's growing Wall-stack thickness T — each Layer's channel depth D and fillet radius R shrinking per the formulas above — while the crossover itself stays pinned at a constant 2.5w below the OML throughout, so the tube's bond line never breaks stride as it's absorbed into the thickening Flange. The same mechanism and view apply to a Lacing anchor entering the zone, using the wider W = 3w channel in place of the Stringer's W = 2w.
...
### Flare Detection
Stringer and Lacing anchor positions are both tested against the Flange's angular extent at every Layer within the Flange zone (the full perimeter, since the Flange spans the whole boundary loop). Any Stringer or Lacing anchor found at a given Layer triggers generation of a Flare Rim at that Layer's innermost Wall, in place of the standard innermost-Wall segment and the standard Trace, per the Flare geometry described under Collision Features. The Rim's width is set to W = 2w for a Stringer anchor or W = 3w for a Lacing anchor; its depth and fillet radius are computed from the current Layer's Flange Wall-stack thickness T via D = 2.5w − T and R = 0.75w × (D / 2.5w). Any other Walls present at that Layer (outer Wall, or buried middle Walls) are generated normally and are not affected. Each Layer's Flare Rim is welded to the one below via the standard Trace crossover mechanism, continuing the tube up through the full Flange zone.
