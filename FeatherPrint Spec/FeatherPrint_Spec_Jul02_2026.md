...
---
*Rick Szalay, 10 June 26 — revised 02 July 26*
...
A Stringer anchor and a Lacing anchor are resolved by the same Flare mechanism; they are not treated as separate collision types. The only difference between the two cases is the flat-bottom width of the canonical Flare Rim profile — 2w for a Stringer, 3w for a Lacing, matching each feature's own perimeter engagement width — which is a parameter of the one Flare geometry rather than the trigger for a second feature. This mirrors how the Conformal Placement Transform already treats CCW and CW Stringer Traces as one geometry with a sign flip, rather than as two features.
...
Unlike the other collision features, the Miter introduces no distinct geometry of its own. It is a rule governing how the Flange's Walls and the Whip's Terminal column interact at the junction, together with a Wall-ordering requirement that makes the rule work (see Wall Print Order).
...
The Flare Rim profile is defined in the Anchor Point UCS, anchored at (0, 0) on the **true OML** — the outer Wall's own toolpath centreline, not the innermost Wall's own surface, and not the raw slice-polygon reference the toolpath generator internally uses for the OML ray-cast (see Implementation Notes: at the first Flange ramp Layer, the outer Wall is a half-width Wall offset outward from that raw reference by w/4, and the Rim's canonical coordinates below must be corrected for that gap; at every other Layer the two coincide).

**Q** is the toolpath-*centreline*-to-toolpath-centreline distance from the OML to the IML — that is, the distance between the outer Wall's own toolpath centre and the innermost Wall's own toolpath centre, at the current Flange Layer. This is *not* the full physical span between the Flange's outer-face and inner-face extrusion surfaces (which is larger by roughly half of the outer Wall's width plus half of the inner Wall's width); using the full physical span in place of Q was an error in the prior revision's implementation, and produced a Rim that never fully closed the gap to the actual IML surface as the Flange thickened. Q follows directly from the Flange's own build-up rule (1.5w total thickness at the first ramp Layer, +0.5w per Layer thereafter) minus the half-widths of the outer and inner Walls at that Layer, and is computed from the Flange's own constructed Wall-stack description rather than re-derived from the build-up formula independently.

Given Q, the profile radius is:

> r = (2.5 − Q) / 2

so that the channel bottom, at y = −Q − 2r = −2.5, always lands a fixed 2.5w below the true OML — the same depth as an unmodified Stringer Trace or Lacing Trace — regardless of how much of that depth the Flange's own Walls already occupy. r shrinks toward zero as Q approaches 2.5w (once the Flange's own Wall stack alone reaches the depth an unmodified Trace would have needed), and is clamped to zero beyond that point.

*d* is the flat span parameter: d = 2.0 for a Stringer anchor, d = 3.0 for a Lacing anchor (matching the Lacing's own total lateral span of 3.0w).

The canonical shape is a mirror-symmetric dovetail cross-section, five segments, traversed **left-to-right** (increasing canonical x) so that it matches the enclosing Wall's own perimeter walk direction — the Wall stops its ordinary walk at the Rim's left (lowest arc-length) end and resumes it at the Rim's right (highest arc-length) end:

| Segment | Centre (w units) | Radius | Direction | Start | End |
| --- | --- | --- | --- | --- | --- |
| Arc 1 | (−d/2−r, −Q−r) | r | CW | 90° | 0° |
| Arc 2 | (−d/2+r, −Q−r) | r | CCW | 180° | 270° |
| Line 1 | — | — | — | (−d/2+r, −Q−2r) | (d/2−r, −Q−2r) |
| Arc 3 | (d/2−r, −Q−r) | r | CCW | −90° | 0° |
| Arc 4 | (d/2+r, −Q−r) | r | CW | 180° | 90° |

Arc 1 sweeps from (−d/2−r, −Q) — the left departure point, flush with the flat Wall line above — down to (−d/2, −Q−r). Arc 2 continues from there to the channel floor at (−d/2+r, −Q−2r). Line 1 crosses the flat bottom to (d/2−r, −Q−2r). Arc 3 sweeps back up to (d/2, −Q−r), and Arc 4 completes the return to the right departure point (d/2+r, −Q), flush with the flat Wall line on the far side. Each arc is tangent to its neighbour, so the whole profile — including its two junctions with the ordinary Wall line above — is smooth.

The canonical profile does not need CW/CCW helix mirroring (see Conformal Placement Transform § CW Helix Mirroring): x_sign = +1 is used unconditionally for both Stringer helix directions and for Lacing. Applying x_sign = −1 would not change the symmetric shape, but would reverse which end is emitted first, breaking the left-to-right traversal the enclosing Wall's walk direction requires.
...
| Whip stub layers (*n*) | Whip / Splay | 3 | layers | Fixed Traces inserted at each Whip endpoint on the n closed-perimeter layers immediately below the first Terminal and above the last, when no coincident Stringer Splay is present. Pending Splay specification; planned to be superseded by the Collar feature. |
...
### Flare Detection
Stringer and Lacing anchor positions are both tested against the Flange's angular extent at every Layer within the Flange zone — the full perimeter for an ordinary (non-Miter) Flange Layer, or the open arc's own span for a Miter Layer. Any Stringer or Lacing anchor found at a given Layer triggers generation of a Flare Rim at that Layer's innermost Wall, in place of the standard innermost-Wall segment and the standard Trace, per the Flare geometry described under Feature Geometry. The Rim's flat-span parameter is set to d = 2w for a Stringer anchor or d = 3w for a Lacing anchor; its arc radius r is computed from the current Layer's Q via r = (2.5 − Q) / 2. Any other Walls present at that Layer (outer Wall, or buried middle Walls) are generated normally and are not affected. Each Layer's Flare Rim is welded to the one below via the standard Trace crossover mechanism, continuing the tube up through the full Flange zone.

Where a Stringer and Lacing anchor's own occupied intervals would overlap once widened to the Flare Rim's flat span (rather than the plain 1w Stringer Trace cutout), the Lacing collision threshold used to decide whether two Stringer anchors should merge into one Rim is itself widened accordingly, so that closely-packed anchors near a tapering cross-section merge into a single, correctly-sized Rim instead of generating two Rims that cross each other.
...
### Conformal Placement Transform
Each canonical point is mapped to world space via the transform described in the Geometric Features section. The `radiusAt(centroid, θ)` method ray-casts from the centroid at angle θ and returns the intersection distance with the perimeter polygon. All feature arc primitives are tessellated at a fixed segment count and each sample is passed individually through the transform. For the Flare Rim, this ray-cast is performed against the OML polygon regardless of which Wall the Rim is being embedded into.
