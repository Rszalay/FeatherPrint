...
| 30 Jun 26 | Third revision — replaced Splay taper/suppression placeholder with finalised proximity-triggered merge design; added Splay Terminal profile geometry |
...
A Splay occurs where a Stringer Trace anchor and a Whip Terminal anchor fall within proximity of each other on the perimeter. The Terminal anchor is fixed at the boundary edge's angular position, while the Stringer anchor advances continuously as the helix ascends; the anchor separation *d* therefore varies from layer to layer as the Stringer approaches, passes near, and recedes from the boundary.

When *d* falls below the proximity threshold, both the standard Trace and the standard Terminal are deleted for that layer, and a Splay Terminal is inserted at the Terminal's anchor position — the same detection-and-substitution pattern used for Lacing, applied here to a Trace/Terminal pair rather than a Trace/Trace pair.

The Splay Terminal is morphologically identical to the standard Terminal, with a single straight segment of variable length *L* spliced into the solid-side arc of the loop. At *L* = 0 the Splay Terminal is dimensionally identical to the standard Terminal. *L* is a function of *d*: it is largest near the proximity threshold, where the Splay must span the full gap between the deleted Trace's and Terminal's footprints to cover both, and shrinks toward zero as *d* decreases and the two footprints converge. The widening is applied only when the colliding Trace anchor lies on the skin side of the Terminal anchor; the exact function relating *L* to *d*, and the treatment of a Trace anchor detected on the hole side, are not yet finalised.

On approach, *d* starts at the proximity threshold (*L* near maximum) and decreases as the Stringer's helix advances toward the Whip, until *L* reaches zero and the Splay holds at standard Terminal size for as long as the Stringer remains in the fully-collided range. On departure, the same function runs in reverse: *L* grows from zero as *d* increases, until *d* exceeds the threshold and the Splay is discontinued, reverting to an independent standard Trace and standard Terminal. Both directions are governed by the same *L*(*d*) function, traversed in opposite order.
...
The Developed view shows the Terminal column stacking across layers at the boundary edge. The perimeter arms extend to the interior side only; the boundary side shows the growing column of stacked Terminal loops. The transparent elements flanking the column represent the bonding substrate at the transition layers above and below the Whip. Where a Stringer Trace anchor comes within proximity of the Terminal anchor on these layers, the Splay collision feature described below provides that substrate directly. Where no Stringer is present in proximity, dedicated fixed Traces are inserted at the Whip endpoint position on the n layers below the first Terminal and the n layers above the last — these are the **Whip support stubs** described in the Parameters table, and remain the bonding mechanism for the no-coincident-Stringer case.

#### Terminal–Stringer Proximity (Splay Trigger)

A Stringer Trace anchor that comes within the proximity threshold of a Whip Terminal anchor triggers a Splay: both the standard Trace and the standard Terminal are deleted for that layer, and a Splay Terminal is inserted in their place, sized by the *L*(*d*) function described in the Splay section below. See Collision Detection in the Implementation Notes for the detection algorithm.
...
![Splay — Profile](images/Splay-Profile.PNG)

The Splay Terminal profile is derived directly from the standard Terminal profile (see Whip, above) by splitting Arc 2 — the single 180° arc forming the bottom of the loop — into two 90° arcs at its deepest point, and inserting a straight segment of variable length *L* between them. Arc 1 and the anchor position are unaffected by *L*; Arc 2b and Line 1 translate outward by *L* along x. At *L* = 0 the two half-arcs rejoin into the original Arc 2 and the profile is dimensionally identical to the standard Terminal.

| Segment | Type | Centre (w units) | Radius | Direction | Start | End |
| --- | --- | --- | --- | --- | --- | --- |
| Arc 1 | Arc | (1.5, −1.5) | 1.5w | CCW | 90° | 180° |
| Arc 2a | Arc | (1.0, −1.5) | 1.0w | CCW | 180° | 270° |
| Insertion | Line | — | — | — | (1.0, −2.5) | (1.0 + L, −2.5) |
| Arc 2b | Arc | (1.0 + L, −1.5) | 1.0w | CCW | 270° | 360° |
| Line 1 | Line | — | — | — | (2.0 + L, −1.5) | (2.0 + L, 0) |

Arc 1 sweeps 90° CCW from the departure point (1.5, 0) to the anchor (0, −1.5), exactly as in the standard Terminal. Arc 2a sweeps 90° CCW from the anchor to the loop's deepest point (1.0, −2.5). The Insertion segment runs horizontally at y = −2.5, extending the loop's solid-side footprint by *L*. Arc 2b sweeps 90° CCW from the end of the Insertion to (2.0 + L, −1.5). Line 1 returns vertically from there to the perimeter surface at (2.0 + L, 0), the far-side return point.

Total loop depth remains fixed at 2.5w for all *L*, matching the standard Terminal and the standard Stringer Trace. Total loop width is (2 + L)w, equal to the standard Terminal's 2w at L = 0.

*L* as a function of anchor separation *d*, and the treatment of a colliding Trace anchor detected on the hole side of the Terminal, are not yet finalised — see the Splay description above and the Collision Detection subsection in Implementation Notes.

##### Slice

*[Drawing: 3D view of a Splay Terminal layer, showing the widened loop replacing both the standard Trace and standard Terminal at the anchor position, with the Insertion segment extending the solid-side footprint toward the deleted Trace's former position.]*

##### Developed

*[Drawing: 3D view of the Splay zone across its layer span, showing the Insertion length *L* shrinking to zero as the Stringer's helix converges on the Whip boundary, then growing back out as it recedes.]*
...
| Whip stub layers (*n*) | Whip / Splay | 3 | layers | Fixed Traces inserted at each Whip endpoint on the n closed-perimeter layers immediately below the first Terminal and above the last, when no Stringer anchor is in proximity for a Splay. |
| Splay proximity threshold | Splay | — | mm/w | Trigger distance *d* for Trace/Terminal merge. Not yet finalised. |
...
For open-manifold layers, an additional proximity test runs between each Stringer Trace anchor and each Whip Terminal anchor on that layer. If the anchor separation *d* is below the proximity threshold, both the Stringer Trace and the Whip Terminal are flagged and deleted, and a Splay Terminal is inserted at the Terminal's anchor position, with Insertion length *L* computed as a function of *d* (see Splay, above). The proximity threshold, the *L*(*d*) function, and the skin-side/hole-side test governing whether widening is applied are not yet finalised.
